  ...
  return (
    <ModalTemplate title={title} onClose={onClose}>
      <form className={styles.newTaskForm} onSubmit={formik.handleSubmit}>
        <TextField label='Title' onChange={formik.handleChange} required />
        <Autocomplete 
          options={top100Films} 
          renderInput={params => <TextField {...params} label='Assign to' onChange={formik.handleChange} required />} 
        />
        <DatePicker label="Due Date" onChange={formik.handleChange} />
        <TimePicker label='Due Time' onChange={formik.handleChange} />
        <Button className={styles.newTaskForm__submit} variant="contained">Submit</Button>
      </form>
    </ModalTemplate>
  )
}
